...
`sp` comes with a more convenient plot command `spplot()`, which takes `Spatial*` objects to plot. `spplot()` is one of the older functions to plot geographic objects.
...
9. Looks better already. But we still have this one area standing out with an extremely high homicide rate, which makes a large part of the map unreadable. So let's change the class intervals to quantiles. We will use `classIntervals` from the `classInt` library, something like:
        ...
        breaks_qt <- classIntervals(vector_of_values_here, n = how_many_classes_?, style = "quantile" [can be omitted -- it is the default])
...
For an introduction to `ggplot` check out [this excellent book by the package creator](http://link.springer.com/book/10.1007%2F978-3-319-24277-4).
...
[^5]: You may still see many examples that use `ggplot2::fortify`. Be aware that that may be deprecated in the future.
...
1. First we use the `get_map()` command to pull down the basemap. We need to tell it the location or the boundaries of the map, the zoom level, and what kind of map service we like (default is Google terrain). That will actually download the tile. `get_map()` returns a ggmap object, name it `ph_basemap`.

    In order to view the map we then use `ggmap(ph_basemap)`.

    Look up the syntax with `?get_map()`, go back and forth between `get_map(..)` and `ggmap(ph_basemap)` to find the correct parameters for our example.
...
ggmap(ph_basemap) +
  ...
  ggtitle("Philadelphia homicide rate per 100,000") # +
    #theme(line = element_blank(),  # don't need this here as ggmap already takes care of most of it
    #    axis.text=element_blank(),
    #    axis.title=element_blank()
    #   panel.background = element_blank()) + 
...
```{r eval=FALSE}
library(tmap)
# Apparently need to read shapefile in again using read_shape from tmap package, 
# otherwise my RStudio blows up. Not clear why, because read_shape also 
# only calls readOGR. Actually, it does some reprojecting! >>> CHECK THIS!! <<<
philly2 <- read_shape("/Users/cengel/Desktop/Philly3/Philly3.shp") 
...
```{r echo=FALSE, message=FALSE}
philly2 <- read_shape("/Users/cengel/Desktop/Philly3/Philly3.shp") # need to read shapefile in again using read_shape from tmap package, otherwise my RStudio blows up. Not clear why, because read_shape also only calls readOGR

philly_tmap <- tm_shape(philly2) +
  tm_polygons("HOMIC_R", style="quantile", title="Philadelphia \nhomicide rate \nper 100,000")
tmap_mode("view")
philly_tmap
```
...
1. Load the `leaflet` library. Use the `leaflet()` function with the Spatial*Object and pipe it to  addPolygons() function. Just to show us a the default map of Philly.
...
7. That's more like it. Finally, I have added for you a control to switch to anotehr basemap and turn the philly polygon off and on. Take a look at the changes in the code below.
