...
If we wanted to add a legend and a title we would recur to basic plot commands to do this.

```{r eval=FALSE}
make.colors <- colorRampPalette(pal)
plot(philly_sf["HOMIC_R"], main="Philadelphia homicide rate per 100,000", col=cut(philly_sf$HOMIC_R, br))
...
**`ggplot` will soon be able to plot `sf` objects directly.** Something like:

    ggplot() + geom_sf(..some_args..)
