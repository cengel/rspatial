...
#. Now write the URL to return the location of Santiago -- what do we get here? 

#. Now we want **_only_** the city in Spain. Extra bonus: Request the results in JSON format.  
If you are interested, here is some documentation: https://developers.google.com/maps/documentation/geocoding/
...
1. Load address data.  
    ...
    The link to the CSV file of the data is this one:
...
```{r eval=FALSE}
dogs_adr <- paste0(dogs$Address, " Austin,TX ", dogs$Zip.Code)
...
4. Save the output as shapefile. 
    ...
    Saving out as csv is pretty easy with `write.table()`. To save the data frame as a shapefile, we convert the dataframe to a `Spatial*Dataframe` first, assign a projection, and then save with `writeOGR()`. Remember that you will need both the `sp` and the `rgdal` libraries for this.
...
5. Now let's use `get_map()` and `ggmap()` to plot the dots over a basemap.

```{r eval=FALSE}
aus <- get_map("Austin, TX")
ggmap(aus) + geom_point(dogs_coords, aes(lon, lat))
```
...
dogs_coords <- geocode(dogs_adr)
aus <- get_map("Austin, TX")
ggmap(aus) + geom_point(data = dogs_coords, aes(lon, lat))
```
...
2. Send each address to the geocoding service.  
We don't have a convenient function to do this, so we have to write our own.
...
This geocoding function unfortunately is not as convenient as the one we used earlier. So we have to loop through our adresses ourselves and save the result to a data frame. Before that you should set `myToken` to the value of your token and make sure that you have the `httr` and `jsonlite` libraries installed.

Once that's taken care of, we can do:

```{r eval=FALSE}
dogs_coords2 <- do.call("rbind", sapply(dogs_adr, function(x) geocodeSL(x, myToken), simplify = FALSE))
```
...
#. Using the provided function `geocodeSL` we can geocode the same adress table as above in a very similar fashion. Give that a try.
...
- [`opencage`](https://CRAN.R-project.org/package=taRifxe=opencage) connects to [OpenCage geocoder](https://geocoder.opencagedata.com/) 
