...
Discuss with your neighbor: What do we need to specify in order to describe these kinds of spatial vector data?
...
Some subsetting:
```{r}
xy_spdf[1:2, ]        # row 1 and 2 only
xy_spdf[["attr2"]]    # column with "attr2" only
```
...
3. Set your working directory to  `R_Workshop` on your desktop
...
spatial_poly <-  SpatialPolygons(list(polys1,polys2,polys3))
...
Add attributes:
```{r}
attr <- data.frame(numbers=1:3, characters=LETTERS[3:1], row.names=c("poly3/4", "poly1", "poly2"))
spatial_polyDf <- SpatialPolygonsDataFrame(spatial_poly, attr)
...
```{r}
as.data.frame(spatial_polyDf) # instad of: spatial_polyDf@data
```
